...
public class ChinaDate {
    private final static String[] N_STRL = new String[]{"", "正", "二", "三", "四", "五", "六", "七", "八", "九", "十", "冬", "腊"};
    /**
     * <lunarInfo数组值计算原理>
     * 0x表十六进制，后五位是十六进制数
     * 例: 1980年数据0x095b0
    ...
    /**
     * 传回y年m月d日对应农历（.year0 .month1 .day2 .yearCyl3 .monCyl4 .dayCyl5 .isLeap6）
     *
     * @param y 年
     * @param m 月
     * @param d 日
    ...
    public static ArrayList<String> getMonths(int year) {
        ...
        for (int i = 1; i < N_STRL.length; i++) {
            baseMonths.add(N_STRL[i] + "月");
        }
        if (leapMonth(year) != 0) {
            baseMonths.add(leapMonth(year), "闰" + N_STRL[leapMonth(year)] + "月");
        }
        return baseMonths;
    }
