...
public class LunarCalendar {
    ...
    /**
     * 表1900到2099年间农历年份相关信息（24位bit的16进制表示）
     * 1. 前4位表该年闰哪月
     * 2. 5-17位表农历年份13个月的大小月分布（0小/1大）
     * 3. 后7位表农历年首（正月初一）对应公历日期
     * <p>
     * 以2014年数据0x955ABF为例：
    ...
    public static int[] solarToLunarDeprecated(int year, int month, int monthDay) {
        ...
        // offset减每农历年天数算当天农历第几天
        // iYear最终结果是农历年份，offset是当年第几天
    ...
    /**
     * 传回农历year年闰哪月（1-12），没闰传0
     *
     * @param year 算的年
     * @return 传回农历year年闰哪个月（1-12），没闰传0
     */
