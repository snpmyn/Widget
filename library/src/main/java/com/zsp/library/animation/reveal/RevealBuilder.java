...
public class RevealBuilder {
    ...
    public void start() {
        ...
        if (this.mChildsAnimation) {
            RevealHelper.findAllVisibleChilds((ViewGroup) this.mViewToReveal, ordoredChildsViews);
        ...
        if (this.mFromView == null) {
            ...
            RevealHelper.translateAndHideView(this.mFromView, this.mViewToReveal, this.mTranslateDuration, this.mCurvedTranslation, this.mCurveControlPoint, this.mHideFromViewAtInterpolatedTime, animationListener);
        }
    }
    ...
    private void revealViewAndChilds(final List<View> ordoredChildsViews) {
        RevealHelper.revealView(mViewToReveal, mRevealDuration, () -> {
                    // - Show childs view if necessary.
                    RevealHelper.orderedShowViews(ordoredChildsViews, mChildAnimationDuration, mDelayBetweenChildAnimation);
